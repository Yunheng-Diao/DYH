...
def partial_autocorrelation(x, param):
    ...
    max_demanded_lag = max([param["lag"]])
    n = len(x)
...
def augmented_dickey_fuller(x, param):
    ...
    autolag = param['autolag']
    adf = compute_adf(autolag)

    if param["attr"] == "teststat":
        res.append((adf[0]))
    elif param["attr"] == "pvalue":
        res.append((adf[1]))
    elif param["attr"] == "usedlag":
        res.append((adf[2]))
    else:
        res.append((np.NaN))
...
def fft_coefficient(x, param):
    ...
    res = [
        complex_agg(fft[param["coeff"]], param["attr"])
        if param["coeff"] < len(fft)
        else np.NaN
        # for config in param
    ]
    # index = [
...
def index_mass_quantile(x, param):
    ...
    if s == 0:
        ...
        return (np.argmax(mass_centralized >= param["q"]) + 1) / len(x)
...
def linear_trend(x, param):
    ...
    return getattr(linReg, param["attr"])
...
def matrix_profile(x, param):
    ...
    key = convert_to_output_format(param)
    feature = param.pop("feature")
    featureless_key = convert_to_output_format(param)
    if featureless_key not in matrix_profiles:
        matrix_profiles[featureless_key] = _calculate_mp(**param)
